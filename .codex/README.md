...
| Bevy runtime naming | Use `AppScene` for the persistent app-level scene, `GameScene`, `DeckScene`, and `DebugScene` for active sub-screen views, `CardModel` for card data, `CardView` for rendered card presentation, and `CardViewBundle` for the visual bundle. |
