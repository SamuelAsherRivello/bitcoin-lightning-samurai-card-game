...
For browser-visible changes, run the real app through `.\Scripts\Common\RunAppWeb.ps1` and inspect it when practical. If the target port is already serving an old build, restart the server before trusting the browser result.
