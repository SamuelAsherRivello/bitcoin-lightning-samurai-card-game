...
| Use `scripts/main/RunTests.ps1` for test execution | The repository requires repeatable scripts under `scripts/`; this script runs the full workspace test suite | Ad hoc `cargo test` only was rejected because the feature explicitly requires a RunTests script |
