...
| Key Legend Label | `DebugHudKeyText.key_code`, `DebugHudKeyText.is_toggle`, `UnderlineColor` | Child text span of the HUD panel | Labels exist for `W`, `A`, `S`, `D`, `F`, and `I`; `WASD` labels are non-toggle and stay inactive |
...
| `W`, `A`, `S`, or `D` pressed | Any diagnostic state | No diagnostic state changes | No gameplay, movement, camera, card, selection, score, deck, or HUD feedback behavior |
