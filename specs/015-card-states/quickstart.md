...
| Inspect opponent reveal | `bevy/crates/game/src/runtime/resources/opponent_match_model.rs` | Confirms current-turn hidden and revealed placement state. |
...
| Drag to location | Drag local hand card to local location slot area | Card occupies first empty local slot and durable zone becomes current-turn location. |
| Return same-turn card | Drag same-turn placed card back to hand area | Slot frees and card returns to chosen hand order. |
| Lock behavior | End turn, then try dragging prior placed card | Drag is rejected; card remains placed. |
| Opponent hidden reveal | CPU/opponent places current-turn card | Non-owner sees back until reveal; owner/rules retain identity. |
...
| ------ | -------------- |
| `card_gesture_update_system` | Replace `active_hand_index` lookup with `CardInteractionModel.active_instance_id`; use `CardInstanceStateModel.zone` to decide whether the source is hand or current-turn location. |
