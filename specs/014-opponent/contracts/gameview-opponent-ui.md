...
| Activation | Cycles to the other mode, saves the selected mode to disk, and starts a fresh game at turn `1/6`. |
...
| Activation | Starts a fresh game in the current mode at turn `1/6`. |
| Reset scope | Clears both players' transient decks, hands, placements, readiness, CPU Brain pending actions, turn state, and winner state. |

## Next / End Turn Control

| Property | Contract |
| -------- | -------- |
| Human control | Lets the near human player mark readiness. |
| Timing | Human player has no timer. |
| Round advancement | Does not advance the turn until both players are ready. |
| Label | May retain existing End Turn wording or use Next, but behavior must be clear. |
...
| Timing | Every visible CPU move or non-move readiness decision is paced by a 0.5 to 1 second delay. |
| Ready signal | CPU readiness is not exposed as a clickable human button. |
| Knowledge | CPU Brain cannot inspect opposing current-turn hidden card identities or unrevealed deck order. |
...
| Progression | Both CPU players continue paced moves or readiness decisions through turns `1/6` to `6/6`. |
...
| Near current-turn placed card | Card front for the near human, hidden from CPU Brain. |
| Far current-turn placed card | Face down until turn end. |
| Any prior-turn placed card | Face up permanently. |
| End-of-turn reveal | All current-turn placed cards flip face up immediately after both players mark Next. |
...
| Property | Contract |
| -------- | -------- |
| Trigger | After both players are ready on turn `6/6`. |
