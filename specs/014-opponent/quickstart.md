...
10. Press the human Next/End Turn control before the CPU is ready and confirm the turn does not advance until the CPU is also ready.
11. While the turn is active, confirm the near player's hand and own current-turn placements appear as card fronts to the human.
12. Confirm the far CPU hand and far current-turn placements appear face down to the human.
13. End the turn and confirm all current-turn placed cards flip face up immediately and remain face up in later turns.
...
15. Observe CPU card moves and readiness decisions: CPU Brain details are not shown, card moves are legal and win-oriented, and each CPU move or decision is paced by 0.5 to 1 second.
16. Play through turn `6/6` and confirm the result identifies exactly one winner with `Status:` text above Mode, such as `Status: Winner is Player 1 (CPU)`.
17. Switch to `CPU versus CPU`, restart, provide no gameplay input, and confirm both players progress automatically through turn `6/6` to final winner status.
...
| CPU Brain Level 1 | Unit tests for legal move selection, win-oriented move preference, seeded random choice among acceptable moves, deterministic same-seed move sequences, 0.5 to 1 second move/decision pacing, no-energy stop, no-legal-move stop, and hidden terminology. |
| CPU Brain Knowledge | Unit tests that Brain can see own hand/open locations/revealed slots but cannot see unrevealed deck order or opposing current-turn hidden placements. |
| Card Visibility | Unit and system tests for face-down opposing hand/current-turn placements and end-of-turn reveal. |
