...
- [X] T005 [P] Add failing model tests for `MatchModeModel`, `MatchPlayerModel`, `MatchRoundModel`, `PlayerController`, `CpuController`, and hidden CPU Brain labels in `bevy/crates/game/src/tests/runtime/resources/opponent_match_model_tests.rs`
...
- [X] T008 Create `bevy/crates/game/src/runtime/resources/opponent_match_model.rs` with `MatchModeModel`, `PlayerController`, `CpuController`, `CpuBrainLevel`, `MatchPlayerModel`, and `MatchRoundModel` primary items using required `HUMAN:` and `AI:` comments
...
- [X] T013 [P] Add failing placement visibility tests for current-round hidden placements, owner-visible fronts, and end-of-round reveal in `bevy/crates/game/src/tests/runtime/resources/opponent_match_model_tests.rs`
- [X] T014 Add `PlacementVisibilityModel` and controller knowledge helpers to `bevy/crates/game/src/runtime/resources/opponent_match_model.rs` so current-round placements hide from opposing controllers and reveal permanently at round end
...
- [X] T029 [US2] Update End Round or Next handling in `bevy/crates/game/src/runtime/systems/mod.rs` so the human near player marks readiness instead of immediately advancing the round
- [X] T030 [US2] Add round readiness resolution in `bevy/crates/game/src/runtime/systems/mod.rs` so rounds 1 through 5 advance only after both near and far readiness flags are set
- [X] T031 [US2] Update round advancement in `bevy/crates/game/src/runtime/systems/mod.rs` so both players receive the scheduled card deal and round energy when a new round starts
- [X] T032 [US2] Update Restart handling in `bevy/crates/game/src/runtime/systems/mod.rs` so it clears both players' decks, hands, placements, readiness, pending CPU state, round state, and winner state
...
**Goal**: After both players are ready on round `6/6`, score all three locations left to right and declare exactly one winner.

**Independent Test**: Complete or simulate a six-round game and verify each location compares top and bottom power totals, awards location ownership, declares near or far winner with no tie result, and updates Status text above Mode with the winning player number and controller type.
...
- [ ] T037 [P] [US5] Add failing readiness-to-winner system tests for round `6/6` and final Status text above Mode in `bevy/crates/game/src/tests/runtime/systems/systems_tests.rs`
...
- [ ] T044 [P] [US3] Add failing CPU Brain knowledge tests proving Brain can inspect own hand, open locations, and revealed slots but cannot inspect unrevealed deck order or opposing current-round hidden placements in `bevy/crates/game/src/tests/runtime/resources/cpu_brain_model_tests.rs`
...
## Phase 6A: User Story 6 - Reveal Current-Round Placements After Next (Priority: P1)

**Goal**: Current-round placed cards are private/face down to the opposing player or controller, then reveal immediately when both players mark Next and stay face up forever.

**Independent Test**: Drag a near card and observe it remains front-facing to the owner but hidden from CPU Brain, observe far CPU current-round cards as face down to the human, verify CPU-owned cards do not respond to mouse hover or rotate toward the cursor, end the round, and verify all current-round placements reveal permanently.

### Tests for User Story 6

- [ ] T053 [P] [US6] Add failing system tests for near owner-visible current-round card fronts, far current-round face-down cards, CPU-owned cards ignoring mouse hover/cursor rotation, and end-of-round flip in `bevy/crates/game/src/tests/runtime/systems/systems_tests.rs`
- [ ] T054 [P] [US6] Add failing model tests that revealed prior-round placements remain visible to both player controllers and CPU Brain in `bevy/crates/game/src/tests/runtime/resources/opponent_match_model_tests.rs`

### Implementation for User Story 6

- [X] T055 [US6] Update card placement logic in `bevy/crates/game/src/runtime/systems/mod.rs` so newly placed cards start as current-round hidden placements for the opposing controller
- [X] T056 [US6] Update card view synchronization in `bevy/crates/game/src/runtime/systems/mod.rs` so near human-owned current-round cards render as fronts to the human while far CPU current-round cards render face down and all CPU-owned cards are excluded from mouse hover, drag affordance, and cursor-facing rotation behavior
- [X] T057 [US6] Update readiness resolution in `bevy/crates/game/src/runtime/systems/mod.rs` so all current-round hidden placements reveal immediately after both players mark Next
- [ ] T058 [US6] Update CPU Brain knowledge construction in `bevy/crates/game/src/runtime/resources/cpu_brain_model.rs` so opposing current-round hidden placements are excluded and prior-round revealed placements are included
...
**Goal**: In `CPU versus CPU`, both near and far players are CPU Brain Level 1 controlled and the game plays automatically from round `1/6` through final winner Status without human gameplay input.
...
- [X] T061 [P] [US4] Add failing system tests for both CPU players moving, waiting for both readiness flags, pacing decisions by 0.5 to 1 second, and reaching round-six winner Status without human gameplay input in `bevy/crates/game/src/tests/runtime/systems/systems_tests.rs`
...
| US5 Resolve Winner | US2 readiness flow | Winner evaluation triggers from round-six readiness. |
| US3 CPU Opponent Actions | US2 player/slot flow | CPU Brain needs real player hands, energy, slots, and readiness. |
| US6 Reveal Current-Round Placements | US2 readiness flow | Reveal happens when both players mark Next. |
...
| 4 | Complete US5 so the six-round game has a final result. |
...
| US5 | Final winner resolution after round six. |
| US6 | Current-round hidden placements and end-of-round reveal. |
...
| Human timing | No human round timer is introduced. |
| CPU undo | CPU players never use Undo; Undo is a human reconsideration action only. |
| Hidden information | Current-round placed cards reveal at end of round and stay face up permanently. |
