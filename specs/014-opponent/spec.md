...
1. **Given** a fresh `Human versus CPU` game starts, **When** round 1 begins, **Then** the near human player and far CPU player each have an independent 12-card copy of the same master deck.
...
1. **Given** a `Human versus CPU` round starts, **When** the far CPU takes a round with at least one move, **Then** the first CPU card move occurs after a 0.5 to 1 second thinking delay.
...
6. **Given** the far CPU has placed a card, **When** that placement is complete, **Then** the CPU does not undo or reconsider that move during the round.
...
2. **Given** a `CPU versus CPU` round begins, **When** both CPU players take their rounds, **Then** each player chooses legal affordable moves intended to improve that player's chance to win and may use randomness when multiple acceptable moves are available.
...
### User Story 6 - Reveal Current-Round Placements After Next (Priority: P1)

In `Human versus CPU`, each player can see their own hand and own current-round dragged cards as card fronts, while the opposing player's hand and current-round placed cards remain face down until both players mark Next. When the round ends, all current-round placed cards flip face up and stay visible for the rest of the game.
...
**Independent Test**: Start a `Human versus CPU` round, drag a near-player card to a bottom slot, observe that the human still sees that card's front while the CPU Brain cannot inspect it, observe the far CPU hand and current-round placements as face-down to the human, then end the round and verify all current-round placed cards reveal face up permanently.
...
3. **Given** the near human drags a card to a location during the current round, **When** the human views that placed card before the round ends, **Then** the human still sees its card front.
4. **Given** the near human has placed a card during the current round, **When** the CPU Brain chooses moves before the round ends, **Then** the CPU Brain treats that near-player current-round card as hidden and cannot use its card identity or values.
5. **Given** the far CPU places a card during the current round, **When** the human views that placed card before the round ends, **Then** the card appears face down.
6. **Given** both players mark Next, **When** the round ends, **Then** all current-round placed cards for both players immediately flip face up.
7. **Given** cards were revealed at the end of a prior round, **When** later rounds begin, **Then** those cards remain face up permanently and may be used as known information for future choices.
...
| --------- | ---------------- |
| The player changes mode during an active game | The active game restarts cleanly in the selected mode at round `1/6` so controller ownership, decks, readiness, and visible slots do not mix old and new mode state. |
| Restart is pressed while a CPU is thinking or moving cards | All pending CPU actions are cancelled and a fresh game starts at round `1/6` in the current mode. |
| The game is in `CPU versus CPU` and the user provides no input | CPU controllers continue paced decisions, readiness, round advancement, and final winner resolution automatically. |
...
| CPU hand cards are still moving into the hand | CPU planning and readiness wait until those cards settle, then wait at least 0.5 seconds before any CPU card can move to a location. |
...
| A card was placed during the current round but the placing player can see its front | That visibility is private to the owning player/controller until the round ends. |
| A card was placed in a prior round | It remains face up forever for both players and for CPU Brain knowledge. |
...
- **FR-006**: Changing mode during an active game MUST start a clean new game in the selected mode at round `1/6`.
...
- **FR-013**: At the start of each round, each player MUST receive cards from that player's own deck according to the existing round card-deal schedule.
...
- **FR-019**: The human-controlled near player MUST be able to signal readiness using the existing end-of-round control, relabeled or interpreted as Next.
- **FR-020**: CPU-controlled players MUST be able to signal readiness without exposing an on-screen button to the human player.
- **FR-021**: A round MUST advance only after both players have signaled readiness.
- **FR-022**: On rounds 1 through 5, resolving both players' readiness MUST start the next round.
- **FR-023**: On round 6, resolving both players' readiness MUST start final winner evaluation and MUST NOT deal additional cards.
...
- **FR-024B**: When cards are dealt into a CPU-controlled player's rendered hand, those hand cards MUST visually come to rest before that CPU player can be considered ready to plan or move cards.
- **FR-024C**: After CPU-controlled hand cards have come to rest, at least 0.5 seconds MUST pass before those cards are eligible to move from hand to a location.
- **FR-025**: A CPU-controlled player's first move, if any, MUST occur only after the CPU hand-settle gate and the 0.5 to 1 second decision delay complete.
- **FR-026**: If a CPU-controlled player makes multiple card moves in one round, those moves MUST be shown one card at a time with a 0.5 to 1 second delay between moves.
- **FR-027**: After a CPU-controlled player has no energy remaining or no legal affordable moves, it MUST signal readiness for the round.
- **FR-028**: A CPU-controlled player that has already signaled readiness for the current round MUST NOT make additional moves until a later round begins.
- **FR-029**: Restart MUST remain available in `Human versus CPU`.
- **FR-030**: Restart MUST start a clean fresh game at round `1/6` using the currently selected mode.
- **FR-031**: Restart MUST clear both players' hands, placed cards, decks, readiness state, pending CPU actions, round state, and winner state.
- **FR-032**: Final winner evaluation MUST occur only after both players are ready on round 6.
...
- **FR-044**: `CpuBrainLevel = 1` MUST choose legal affordable drag-equivalent moves during a round until it has no energy remaining or no legal affordable moves.
...
- **FR-045**: No round timer MUST be imposed on the human player; the human player may take as long as desired before signaling readiness.
...
- **FR-054**: CPU Brain knowledge MUST NOT include opposing current-round placed card identities or values until those cards are revealed at the end of the round.
...
- **FR-057**: A player's cards placed during the current round MUST be hidden from the opposing player/controller until the round ends.
- **FR-058**: A player's own cards placed during the current round MAY still appear as card fronts to that player.
- **FR-059**: When both players have marked Next and the round ends, all current-round placed cards MUST immediately reveal face up.
- **FR-060**: Once a placed card has been revealed at the end of a round, it MUST remain face up permanently for the rest of the game.
...
- **FR-064**: In `CPU versus CPU`, a fresh game MUST progress from round `1/6` through final winner status without requiring human card movement, Next input, or other gameplay input.
...
| **Readiness State** | Per-player round state indicating whether that player has signaled readiness to advance. |
| **CPU Brain** | Authored game-code logic that controls a CPU player without using runtime generative AI. |
| **CPU Brain Knowledge** | The subset of match information available to CPU Brain: its hand, open locations and abilities, and revealed slots on both sides; it excludes unrevealed deck cards and opposing current-round hidden placements. |
...
| **CPU Round Plan** | The CPU Brain's selected sequence of card moves for the current round before it signals readiness. |
| **Hidden Current-Round Placement** | A card placed into a location during the current round that is private to its owner and face down to the opposing player/controller until the round ends. |
| **Revealed Placement** | A placed card that has flipped face up at end of round and remains visible to both players and CPU Brain thereafter. |
...
| **Match Winner** | The player who wins two or more locations after round 6 resolves. |
...
- **SC-005**: In at least 95% of observed CPU rounds, CPU moves and readiness decisions are separated by 0.5 to 1 second delays.
- **SC-005A**: In 100% of seeded CPU Brain tests, the same starting state and same seed produce the same move sequence.
- **SC-006**: In 100% of readiness tests, the game remains on the current round until both players have signaled readiness.
- **SC-007**: In 100% of restart tests, the game returns to round `1/6`, clears both players' transient game state, and keeps the selected mode.
- **SC-008**: In 100% of completed six-round games, final evaluation checks three locations from left to right and declares exactly one match winner.
...
- **SC-011**: In 100% of current-round visibility checks, each player can see their own current-round placed cards while the opposing controller cannot inspect those card identities or values until end-of-round reveal.
- **SC-012**: In 100% of end-of-round reveal checks, all current-round placed cards flip face up immediately and remain face up in later rounds.
...
- The existing lower-right End Round control from Spec 007 may be relabeled to Next, or may retain its current label while serving the readiness function, as long as the player can clearly signal readiness.
- The phrase "round" in this spec maps to the existing six-step round flow from Spec 007; round `1/6` through `6/6` are the visible progression states.
...
- Current-round card placement uses hidden information even if the owning human sees that card's front for usability.
- Future specs may add cards that remain face down for longer, but this feature reveals all current-round placements when the round ends.
- Undo is retained for human reconsideration; CPU players are assumed not to change their mind during a round.
