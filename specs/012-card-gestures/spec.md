...
**Created**: 2026-05-11  
**Status**: Draft  
**Input**: User description: "Card gestures for the game view. Mouse input should also mean mobile pointer input such as tap, press, swipe, and drag. Clicking a hand card should no longer switch to the card browser scene. A click selects the card in the game view, moves it to the center at the same large inspection position used by the card browser scene, around 90% of screen height, and clicking that selected card returns it to its hand position. Dragging a hand card after moving a few pixels should not count as a click. Dragging allows the card to move into empty slots in the local player's side of the three location areas. Each location has four slots above and four below, and the local human player may only allocate into the twelve closest slots below the three locations. Dropping onto an empty valid slot snaps the card there while preserving card aspect ratio and fitting the slot."

## User Scenarios & Testing *(mandatory)*

### User Story 1 - Inspect a Hand Card In Place (Priority: P1)

A player can tap or click a card in their hand to inspect it at the center of the game view without leaving the game view. The enlarged card appears in the same center inspection position used by the card browser view, sized to nearly fill the available height while preserving its aspect ratio.
...
2. **Given** a hand card is selected for inspection, **When** its movement completes, **Then** it is centered in the screen at the same inspection position as the card browser view and is approximately 90% of the safe visible height.
...
4. **Given** a hand card is clicked or tapped for inspection, **When** the interaction resolves, **Then** the card browser view is not opened.
...
2. **Given** the player presses a hand card and moves beyond the click movement threshold before release, **When** the gesture continues, **Then** the gesture is treated as a drag.
...
1. **Given** the game view has three location areas with four local-player slots below each location, **When** the player drags a hand card over an empty local-player slot and releases, **Then** the card snaps into that slot.
2. **Given** a hand card is dropped into a valid empty local-player slot, **When** placement completes, **Then** the card is sized to fit the slot while maintaining its card aspect ratio.
...
- **FR-004**: Selecting a hand card for inspection MUST NOT switch to, open, or otherwise activate the card browser view.
- **FR-005**: A selected inspected card MUST move to the center inspection position used by the card browser view while the game view remains active.
...
- **FR-011**: Dragging a hand card MUST let the player preview movement over the game view while preserving card aspect ratio.
...
- **FR-018**: A card snapped into a local-player slot MUST shrink or expand to fit that slot while preserving card aspect ratio.
...
- **SC-001**: In 100% of click or tap inspection tests on hand cards, the game view remains active and the card browser view is not opened.
...
| The center inspection position should match the already approved card browser inspection pose as closely as the game view layout allows. | The request wants the exact same position as the card browser scene while staying in the game view. |
