...
| `CardSlotRect` | `left`, `top`, `width`, `height` | Shared safe-GameScene slot geometry used by debug drawing, hit testing, and placement transforms | Runtime slot geometry matches the debug-drawn reference lines |
...
| Resolve click/tap | Pointer release before crossing threshold | Card becomes `SelectedInspecting` in `GameScene` |
...
| GameScene hand-card click/tap selects for in-place inspection and never opens DeckScene. | FR-003, FR-004, FR-005 |
