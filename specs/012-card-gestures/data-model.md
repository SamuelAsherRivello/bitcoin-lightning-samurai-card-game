...
| `CardStateModel` | card index to `CardState` mapping | Tracks whether each playable card is in hand, being dragged, or placed at a location | Only `Hand` cards can begin a drag |
| `CardState` | `Hand`, `Dragging`, `Location` | Runtime gameplay state for a card during this feature | `Dragging` is temporary during an active drag; `Location` cards are not draggable |
...
| Press non-hand card | Pointer press on card with `CardState` other than `Hand` | Press is ignored for drag start |
...
| Only cards in `Hand` state can begin drag gestures. | FR-031 |
