...
| Press locked location card | Pointer press on a card in `LocationLocked` state | No drag gesture starts | Cards from prior rounds are locked after End Turn |
...
| `LocationCurrentRound` | ✅ | Card was placed in a location this round and may return to hand before End Turn |
| `LocationLocked` | ❌ | Card remained placed after End Turn and cannot be moved by drag |
...
| Prior-round locked card | Press or drag a card that remained placed after End Turn | Drag does not start and card remains placed |
