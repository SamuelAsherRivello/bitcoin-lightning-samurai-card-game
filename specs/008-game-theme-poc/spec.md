...
**Created**: 2026-05-10  
**Status**: Draft  
**Input**: User description: "Create a visual and interaction proof-of-concept for a stylized tactical card game currently titled 'Card Katana'. The title is temporary and should not yet appear in namespaces, architecture, or technical systems. This prototype is focused entirely on atmosphere, visual identity, interactivity, and thematic cohesion rather than gameplay implementation. Theme is JW, Japanese Warriors, with Bamboo Forest and Coastal Harbor worlds, reusable tactical locations, four bottom cards, cursor/touch card tilt, card selection into a Deck, card flipping, card theme cycling, and strict separation between world themes and card themes. The art style is Japan Realism: cinematic feudal Japanese realism, chiaroscuro lighting, realistic materials, mature tactical tone, subtle fog/rain/smoke/natural lighting, and no visible magic or glowing fantasy effects."

## Clarifications

### Session 2026-05-10

- Q: What changes when the Deck changes the current card theme? → A: CardUI settings are stored globally and applied to all cards; the flip button only flips the currently viewed browser card and does not persist flip state.
...
A player clicks or taps one of the four permanent game-scene cards to enter a focused Deck for that specific card, where the selected card is enlarged and can be flipped temporarily or adjusted through global CardUI settings without changing the match world.

**Why this priority**: The Deck proves individual character presentation, card flipping, and card theme independence from the active world.
...
1. **Given** the game scene is visible, **When** the player clicks or taps Kage Ren, Lord Daichi, Sister Hotaru, or the placeholder Yokai card, **Then** the Deck opens focused on that selected card.
2. **Given** the Deck is open, **When** the player triggers a card flip, **Then** the selected enlarged card flips between front and back presentation.
3. **Given** the Deck is open, **When** the player presses `T`, **Then** the stored CardUI settings change and the current card visuals update as appropriate.
4. **Given** the Deck changes CardUI settings, **When** the player views any card, **Then** those settings are applied consistently to all cards.
5. **Given** the Deck flips the current card, **When** the player leaves or selects another card, **Then** the flip state is not stored as a durable card state.
6. **Given** the Deck changes CardUI settings, **When** the player returns to the game scene, **Then** the active world remains unchanged.
...
- If a card is selected while cards are tilted toward the cursor, the Deck opens the selected card without carrying over an exaggerated tilted pose.
...
- If `T` is pressed rapidly in the Deck, only global CardUI settings change and the match world remains stable.
- If a card is flipped in the Deck and the player opens another card, the newly opened card is not required to inherit the prior card's temporary flipped state.
...
- **FR-006a**: Each world background MUST make the six card-slot areas easier to perceive through grounded, diegetic surface treatment such as submerged planks, stone beds, cleared silt, reeds, moss, shadow, water depth, or other believable scene materials; it MUST NOT use visible UI borders, labels, guide lines, or repeated non-diegetic marks baked into the art.
- **FR-006b**: Each world background MUST align those six diegetic slot treatments to the runtime `CardSlotBoardModel` slot-area rectangles, not to hand-estimated screenshot pixels. In GameScene logical coordinates, the slot areas are `(364,44,184,180)`, `(548,44,184,180)`, `(732,44,184,180)`, `(364,432,184,180)`, `(548,432,184,180)`, and `(732,432,184,180)`. For the current `1600 x 1000` world background texture rendered with `GAME_SCENE_WORLD_BACKGROUND_BLEED = 1.18`, those correspond to texture rectangles `(508,123,195,191)`, `(703,123,195,191)`, `(897,123,195,191)`, `(508,534,195,191)`, `(703,534,195,191)`, and `(897,534,195,191)`, rounded to the nearest pixel. No slot-like clearing, platform, plank bed, stone bed, or other card-placement cue may appear outside those six areas.
...
- **FR-018**: Clicking or tapping a card MUST open a Deck focused on that specific card.
- **FR-019**: The Deck MUST enlarge the selected card and focus on one card at a time.
- **FR-020**: The Deck MUST support flipping the selected card between front and back presentation.
- **FR-021**: Pressing `T` in the Deck MUST change stored CardUI settings and update card visuals as appropriate.
- **FR-022**: CardUI settings MUST be stored globally and applied to all cards rather than stored independently per card.
- **FR-023**: The Deck flip control MUST only flip the currently viewed card for animation testing and MUST NOT store durable flip state.
...
- **FR-045**: Title art MAY be positioned at the bottom, top, or slightly off-center as long as it supports the card identity, remains readable at game-scene and Deck sizes, and never clips against the `840 x 1440` image borders.
...
- **FR-051**: Card character silhouettes and stances MUST be readable at game-scene card size and enlarged Deck size.
...
- **FR-055**: Completing this proof-of-concept MUST permanently change the game scene to use the four-card bottom lineup and selected-card Deck flow.
...
- **Deck**: A focused card viewing mode that enlarges one selected card and supports temporary flipping plus global CardUI setting changes.
...
- **SC-005**: A tester can open the Deck from any of the four cards in no more than one click or tap.
- **SC-006**: In 10 consecutive presses of `T` inside the Deck, global CardUI settings change without changing the active game-scene world.
...
- **SC-010**: At least 4 out of 5 testers can read the primary stance and silhouette of each named character at both bottom-row card size and enlarged Deck size.
- **SC-011**: After changing CardUI settings in the Deck, a tester can view multiple cards and confirm the same CardUI settings apply to all cards.
- **SC-012**: After flipping a card in the Deck and opening another card, a tester can confirm the prior flip state was not stored as durable card state.
...
- The Deck includes at least two CardUI setting states so that pressing `T` visibly changes card presentation.
