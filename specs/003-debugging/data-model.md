...
| ------ | ------ | ------------- | ---------------- |
| Debugging Tooling | DebugHUD, inspector, Card UI separation, debug drawings, terminal logs, test workflow | Supports developer-facing diagnostics across rendered UI, terminal output, and automated checks | Must remain developer-facing and must not become final player-facing gameplay UI |
...
| Card UI | Card-specific temporary controls and labels | Lives with card-specific prototype behavior and remains conceptually separate from DebugHUD | Must not be merged into DebugHUD or described as final player-facing UI |
| Debug Drawing | Runtime visual mark geometry, target area metadata, temporary label or marker intent | Marks requested game scene areas for shared visual discussion and QA | Must remain temporary, scoped to the requested scene area, and removable or replaceable with real UI/art later |
