...
### User Story 3 - Keep Visual Modifier Rules Centralized (Priority: P3)

As a developer, I need visual modifier rules represented as a focused VMS layer instead of scattered ad hoc color changes so later card, location, and score modifiers can reuse the same presentation contract.

**Independent Test**: A developer can identify one model/component surface that declares active visual modifiers and one system path that synchronizes modifier outlines to point view visuals.
...
| Both modifiers apply to different point views in one location | Card point views may show `abilityoutline` while the leading location total point view shows `leadingscoreoutline`; these are independent. |

## Requirements

| ID | Requirement |
| -- | ----------- |
| FR-001 | The system MUST define a Visual Modifier System concept that can assign named visual modifiers to point views. |
| FR-002 | The system MUST define `abilityoutline` as a gold outline around the circular background behind a card point value. |
| FR-003 | The system MUST apply `abilityoutline` to card power point views when an active non-zero ability modifies that card's displayed power. |
| FR-004 | The system MUST remove `abilityoutline` from card power point views when no active non-zero ability modifies that card's displayed power. |
| FR-005 | The system MUST define `leadingscoreoutline` as a white outline around the circular background behind a location total point value. |
| FR-006 | The system MUST apply `leadingscoreoutline` only to the higher of the two location total point views within the same location. |
| FR-007 | The system MUST remove `leadingscoreoutline` from both location total point views when their values are equal. |
...
| Entity | Description |
| ------ | ----------- |
| `VisualModifier` | Named presentation state that can be applied to a point view, initially `AbilityOutline` and `LeadingScoreOutline`. |
...
| `abilityoutline` | Gold outline shown on a card power point circle when an active ability modifies that card power. |
| `leadingscoreoutline` | White outline shown on the location total point circle that is higher than its paired total in the same location. |
