...
- Q: What default artwork should the polished card use? -> A: Use the requested initial CardTheme: a blue-and-white repeated icon-like cloud background, a subtle 45-degree grey/off-grey pinstripe frame, a flat superhero-inspired foreground character that breaks out of the frame, and a title layer in front of the foreground that also partially breaks out of the frame.
...
- Q: How should `005-card-polish` separate artwork from card behavior? -> A: Treat the card's layer order, parallax, masking, and HUD interaction as `CardStructure`; treat all artwork and visual-style assets applied to that structure as `CardTheme`.
...
- Q: What is the initial `CardTheme`? -> A: A generated-texture theme with blue-and-white repeated icon-like clouds for the background, subtle 45-degree grey/off-grey pinstripes for the frame, a flat superhero-inspired foreground character that breaks out of part of the frame, and a title that sits in front of the foreground and also partially breaks out of the frame.
- Q: How should texture artwork be produced for the initial theme? -> A: Use static generated image textures for the background, frame, and foreground artwork instead of manually constructing dot patterns or placeholder primitives; each visual style includes a texture material shader.
- Q: How should the frame be represented structurally? -> A: CardStructure defines one frame object with a central cutout stencil/aperture and continuous full-card UVs; CardTheme only chooses the frame texture applied to that one object.
- Q: How should theme switching work initially? -> A: Add a HUD `T` key that toggles through the CardTheme registry sized for two themes; with SkyBolt and Tar available, the toggle cycles between those two themes and keeps the active artwork valid.
...
### User Story 5 - Toggle Card Theme Slot (Priority: P3)

A reviewer can press the HUD `T` key to cycle the card theme selection, so the polish system is ready for a second theme without changing the card structure.

**Why this priority**: Theme switching proves the artwork is organized as interchangeable `CardTheme` data instead of being fused into the structural parallax behavior.

**Independent Test**: Launch the prototype, press `T`, and verify the HUD action is recognized while the card remains on the requested initial theme when no second theme is available.

**Acceptance Scenarios**:

1. **Given** the prototype has only the requested initial CardTheme available, **When** the reviewer presses `T`, **Then** the card remains visually stable and does not enter an invalid or blank theme state.
2. **Given** the HUD is visible, **When** the reviewer presses `T`, **Then** the HUD communicates the active CardTheme state clearly enough for testing.
3. **Given** SkyBolt and Tar CardThemes are available, **When** the reviewer presses `T`, **Then** the same control cycles between the two theme entries without changing the CardStructure.
...
- If the reviewer presses `T`, the card should cycle between available themes and should not flash, disappear, or show missing artwork.
...
- **FR-005**: The requested initial `CardTheme` MUST apply generated static artwork assets to the CardStructure rather than relying on manually constructed dot patterns, manually assembled primitive pattern elements, or plain placeholder rectangles as the final theme artwork.
...
- **FR-007**: The requested initial CardTheme's background visual style MUST use a static generated texture with repeated icon-like clouds in blue and white, and it MUST include a texture material shader.
...
- **FR-010**: The requested initial CardTheme's frame visual style MUST use a static generated texture with subtle grey and off-grey pinstripes angled approximately 45 degrees upward to the right, and it MUST include a texture material shader.
...
- **FR-012**: The requested initial CardTheme's foreground visual style MUST use a static generated texture of a flat superhero-inspired character, using the attached example's character fit and cape only as inspiration, and it MUST include a texture material shader.
...
- **FR-026**: The requested initial CardTheme's title artwork MUST partially break out over the frame in a premium trading-card style while remaining visually associated with the card.
- **FR-027**: The application MUST define a CardTheme registry intended to hold two theme entries for this feature phase, with the requested initial CardTheme as the first available theme.
- **FR-028**: The DebugHUD MUST include a `T` key interaction that toggles the active CardTheme selection through the registry.
- **FR-029**: Pressing `T` MUST keep the card on a valid available CardTheme and MUST NOT show missing, blank, or invalid artwork.
- **FR-030**: CardTheme artwork MUST be replaceable without changing CardStructure behavior, including layer order, clipping, apparent depth rules, parallax response, and HUD theme toggle behavior.
- **FR-037**: The frame layer MUST be one CardStructure object with a center cutout and continuous frame UV mapping, so a CardTheme frame texture appears continuous rather than as four separately mapped border objects.
...
- **CardStructure**: The reusable card presentation structure that owns layer order, clipping, apparent depth, parallax response, frame shine behavior, and HUD theme switching independent of any specific artwork.
- **CardTheme**: The interchangeable artwork package applied to a CardStructure, including generated static textures and texture material shader choices for the background, frame, foreground, and title presentation.
...
- **Frame Layer**: The card border or framing surface that acts as the visual reference plane, includes the hole that reveals the background, carries the holographic shine, and receives the CardTheme frame visual style.
...
- **Visual Style**: The CardTheme-provided look for a layer, including its generated static texture and the texture material shader used to display it.
...
- **Theme Toggle Input**: The HUD `T` key interaction that cycles the active CardTheme selection.
...
- **SC-004**: In neutral view, the requested initial CardTheme reads as blue-and-white repeated icon-like clouds in the background, subtle grey/off-grey diagonal pinstripes in the frame, a flat superhero-inspired foreground character, and a title in front of the character.
...
- **SC-012**: Pressing `T` with only the requested initial CardTheme available produces no missing-art, blank-art, crash, or invalid-theme state in 100% of tested attempts.
...
- **SC-014**: Pressing `H` toggles only hot-reload auto-restart, restores through local persisted DebugHUD input state, and does not affect card theme selection in 100% of tested attempts.
...
- The requested initial CardTheme replaces previous placeholder artwork assumptions, including the white frame and rectangle foreground.
...
- The attached example image is inspiration for character fit, cape placement, and premium breakout composition only; the requested initial CardTheme should not copy protected character identity, logos, or exact artwork.
- The first polished version should use generated static theme textures for the requested background, frame, and foreground rather than manually drawn dots or primitive placeholder geometry.
