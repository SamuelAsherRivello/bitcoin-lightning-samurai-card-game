...
**Created**: 2026-05-12
**Status**: Draft
**Input**: User description: "Update the Deck scene to show deck collections and cards, persist deck data, and have GameScene draw and consume cards from a copied player deck per game session."

## User Scenarios & Testing *(mandatory)*

### User Story 1 - Manage Deck View Layout (Priority: P1)

A player opens the Deck and sees two clear areas: a left deck list and a right deck contents panel. The deck list includes one selectable deck named `Deck01`, and only deck cards are shown on the right. The existing card UI overlay is not shown in this screen.
...
**Independent Test**: Launch Deck from a fresh run and verify the two-area layout renders with one deck button and exactly one set of card visuals on the right for that deck, with no card inspection UI shown.

**Acceptance Scenarios**:

1. **Given** Deck is active, **When** the scene renders, **Then** the left side contains a deck list with a button labeled `Deck01`.
2. **Given** `Deck01` is selected, **When** the deck panel loads, **Then** the right side displays all cards in that deck.
3. **Given** the player opens the Deck scene, **When** they look at the main panel, **Then** no card inspection UI is visible there.
...
**Independent Test**: Remove saved deck data and restart. Open Deck; verify `Player 01` has `Deck01` with 12 cards generated from 4 base cards in a randomized order, and confirm changes are preserved after restart.
...
2. **Given** a player has 12 card entries in `Deck01`, **When** they reopen Deck, **Then** the same deck and card count are loaded from disk.
...
| More than 12 cards are somehow persisted | The game copy uses persisted ordering and still consumes cards from that full list; Deck continues to reflect the full persisted list. |
| User opens Deck during or after an active game | Deck uses the persisted player deck, while active game continues to use the previously copied game deck.

## Requirements *(mandatory)*

### Functional Requirements

- **FR-001**: Deck MUST split its main content into a left deck list region and a right deck contents region.
...
- **FR-004**: Deck MUST NOT display the Card UI overlay controls used in the gameplay scene.
...
- **FR-016**: Deck MUST read from the persisted deck collection, not directly from the active game deck.
...
- **SC-001**: In 100% of Deck launches after the change, users can see both deck list and deck contents panels, with `Deck01` available in the left panel.
...
- **SC-003**: In 95% of replayable save/restart checks, Deck shows the same persisted `Deck01` deck identity and card count after relaunch.
...
- Reordering logic in Deck is deferred unless later features explicitly add editing operations beyond default generation and persistence.
