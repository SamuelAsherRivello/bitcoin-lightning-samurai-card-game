...
**Goal**: A developer can distinguish app structure, card data, and rendering through `AppScene`, `GameScene`, `DeckScene`, `DebugScene`, `DebugScene`, `CardModel`, `CardView`, and `CardViewBundle`.
...
- [X] T023 [P] [US3] Add or update scene/view structure tests in `bevy/crates/game/src/runtime/systems/mod.rs` to keep `AppScene` always present while one of `GameScene`, `DeckScene`, or `DebugScene` is active
...
- [X] T025 [US3] Rename or document scene and sub-screen presentation helpers in `bevy/crates/game/src/runtime/components/mod.rs`, `bevy/crates/game/src/runtime/resources/mod.rs`, and `bevy/crates/game/src/runtime/systems/mod.rs` toward `AppScene`, `ActiveView`, `GameScene`, and `DeckScene` after approval
...
- [X] T027 [US3] Document the `AppScene`, `GameScene`, `DeckScene`, `DebugScene`, `DebugScene`, `CardModel`, `CardView`, and `CardViewBundle` concepts and required contents in `README.md`
...
- [X] T037 Perform the quickstart behavior smoke in `specs/009-theme-reorganization/quickstart.md` for `GameScene` display, world toggle, card click, card flip, and CardUI toggle
...
| US3: Use Purposeful Scene and Model/View Naming | Phase 2 and card paths from US1 | Documentation can start early; final traceability depends on approved `AppScene`, `GameScene`, `DeckScene`, `DebugScene`, `DebugScene`, `CardModel`, and `CardViewBundle` names. |
...
Task: "T023 [P] [US3] Add or update scene/view structure tests in bevy/crates/game/src/runtime/systems/mod.rs to keep AppScene always present while one of GameScene, DeckScene, or DebugScene is active"
```
...
3. Deliver US3 to complete `AppScene`, `GameScene`, `DeckScene`, `DebugScene`, `DebugScene`, `CardModel`, `CardView`, and `CardViewBundle` terminology and traceability.
...
- Use `AppScene` for the always-present app-level scene; use `GameScene`, `DeckScene`, and `DebugScene` for active sub-screen presentations; use `CardModel` for card data, `CardView` for rendered presentation, and `CardViewBundle` for the visual bundle that creates card visuals after approval.
