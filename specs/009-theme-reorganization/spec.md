...
**Independent Test**: Can be fully tested by reading the updated documentation and validating that the persistent scene is described as `AppScene`, active sub-screen presentations are described as `GameScene`, `DeckScene`, and `DebugScene`, card data is described as `CardModel`, card rendering is described as `CardView`, and the card visual spawn bundle is described as `CardViewBundle`.
...
2. **Given** a developer reads active screen documentation, **When** the gameplay and deck presentations are introduced, **Then** they are called `GameScene`, `DeckScene`, and `DebugScene`.
...
- If the app usually shows one page at a time, `AppScene` remains the always-present exception while `GameScene`, `DeckScene`, or `DebugScene` is the active sub-screen loaded on top.
...
- **FR-012**: The gameplay and Deck sub-screen presentations MUST be documented as `GameScene`, `DeckScene`, and `DebugScene`.
...
- **FR-018**: Legacy names such as `ActiveScene`, `GameSceneRoot`, `GameSceneEntity`, `DeckSceneRoot`, `DeckSceneEntity`, `CardType`, `CardTypeRegistry`, and `ActiveCardType` MUST NOT remain after implementation except in migration notes or historical references.
...
- **FR-022**: Existing card-facing user behavior from the current proof-of-concept MUST remain unchanged after reorganization, including bottom-row card display, card selection, Deck viewing, and card flipping.
...
- **GameScene**: The gameplay sub-screen presentation loaded on top of `AppScene`.
- **DeckScene**: The Deck sub-screen presentation loaded on top of `AppScene`.
...
- **SC-004**: A developer can identify the difference between `AppScene`, `GameScene`, `DeckScene`, `DebugScene`, `DebugScene`, `CardModel`, `CardView`, and `CardViewBundle` from documentation within 2 minutes.
